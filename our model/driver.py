...
#
parser.add_argument('--lr_update', type=int, nargs='+', default=[60, 160, 300, 500], help='decrease learning rate at these epochs')
#
parser.add_argument('--epochs', type=int, default=600, metavar='N', help='number of epochs to train (default: 10)')
#
parser.add_argument('--dataset', type=str, default='lorenz', metavar='N', help='dataset name')
#
...
#
parser.add_argument('--wd', type=float, default=0.015, metavar='N', help='weight_decay L2 regulization (default: 1e-5)')
#
parser.add_argument('--gradclip', type=float, default=8.0e-8, help='gradient clipping')
#
...
#******************************************************************************
X,Ydim = data_from_name(args.dataset,args.dim,args.feature,args.noise)
...
Y = X[:,args.dim if args.feature == 0 else Ydim]
Ytrain = []
j = 0
...
score = torch.zeros((1,1,dx,dx))
for i in np.arange(0,args.dy-1,1):
    Xinput = Xtest[i:i+1,:,:,:].float()

    Ypred,scores = model(Xinput,Ytmp.float())
    Ytmp = torch.cat((Ytmp[:,:,:,1:args.dy-1],Ypred[:,:,:,args.dy-1:args.dy]),dim=3)
    score = score + scores

''' 
if args.feature == 0:
	score = score.detach().numpy().reshape( (score.shape[2],score.shape[3]) )
	score = score[args.dim,:]/args.dy
	sorted_idx = np.argsort(score)
	sorted_idx = sorted_idx[::-1]

	data_df = pd.DataFrame(sorted_idx)
	data_df.to_csv(args.dataset + str(args.dim) + "_feature.csv")
'''
